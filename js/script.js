	...
	htmlString() {
		...
		let htmlResult = 
			`<img 
				...
				src="waterpipesassets/pipe${capitalizedType}.svg"
...
game.chooseSettings = () => {
	$('.settings form').on('submit', function(e) {
		...
		game.playSound('waterpipesassets/place.wav');
...
game.lose = ([x, y]) => {
	...
	// Play sound on lose
	game.playSound('waterpipesassets/water.wav');
...
game.win = () => {
	// Play sound on win
	game.playSound('waterpipesassets/win.wav');
...
game.intervalTimer = (interval) => {
	...
	setInterval(() => {
		...
		if (game.timeToStart <= 0) {
			...
			if (!waterStarted) game.playSound('waterpipesassets/valve.wav');
			waterStarted = true;
		};
	}, interval);
};
